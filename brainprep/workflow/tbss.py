...
def brainprep_tbss_preproc(outdir, fa_file=None, data=None, bvecs=None,
                           bvals=None, mask=None, target=None):
    ...
    if data is not None:
        ...
        (md_file, fa_file, s0_file, tensor_file, m0_file,
         v1_file, v2_file, v3_file, l1_file, l2_file,
         l3_file) = brainprep.dtifit(
    ...
    elif fa_file is None:
        raise ValueError("Specify either DWI or FA inputs.")
...
def brainprep_tbss(outdir, use_fmrib58_fa_mean_and_skel=True, target=None,
    ...
    if target_skel is not None:
        ...
        mean_fa_root = os.path.join(outdir, "stats", "mean_FA")
        cmd = ["fslmaths", target, "-mas", mean_fa_mask, mean_fa_root]
        execute_command(cmd)
        mean_fa_skel_root = os.path.join(outdir, "stats", "mean_FA_skeleton")
